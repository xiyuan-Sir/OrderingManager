...
function() {
	...
	var okTab = layui.okTab({
		// 菜单数据源
		//url: "data/navs.json",
		data: JSON.parse(sessionStorage.getItem('loginUserInfo')).Menus,
		// 允许同时选项卡的个数
		openTabNum: 30,
	...
	//JSON.parse(sessionStorage.getItem('loginUserInfo')).LoginName)
	$("#welcomelogin").text('你好,' + JSON.parse(sessionStorage.getItem('loginUserInfo')).LoginName);
var config = okUtils.local("okConfig") || okConfig || {}; objOkTab = okTab; okLoading && okLoading.close();
/**关闭加载动画*/

$(".layui-layout-admin").removeClass("orange_theme blue_theme"); $(".layui-layout-admin").addClass(config.theme);
...
$("body").on("click", "#okSetting", function() {
	layer.open({
		...
		time: 200000, //2秒后自动关闭
		anim: -1,
		content: "./Views/System/setting.html"
	});
});
 ...
 * 锁定账户
 */
var lock_inter = ""; lockShowInit(okUtils); $("#lock").click(function() {
...
function lockShowInit(okUtils) {
	...
	$("#lockQuit").click(function() {
		window.location.replace("login.html"); //替换当前页面
	});
}
});
